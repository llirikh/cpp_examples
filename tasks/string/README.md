...
В вашем файле должна отсутствовать функция ```main()```, а сам файл должен называться ```string.hpp```. В качестве компилятора необходимо указывать ```MakeC++17```. Ваш код будет вставлен посредством команды ```#include "string.hpp"``` в программу, содержащую тесты; вследствие этого код необходимо отправлять в файле со строго соответствующим именем!