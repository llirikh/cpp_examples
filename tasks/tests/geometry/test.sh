...
clang-tidy -extra-arg=-std=c++20 -quiet geometry.hpp
if [[ ! $? -eq 0 ]]
then
  echo " "
...
clang-format -i -style='{BasedOnStyle: Google, DerivePointerAlignment: false, PointerAlignment: Left, AlignOperands: true}' geometry.hpp &&
git diff --ignore-submodules --color geometry.hpp > diff &&
cat diff
if [[ -s diff ]]
then
  echo " "
...
clang++ -std=c++20 -pedantic -Werror -Wextra -fsanitize=address -fsanitize=undefined test.cpp -o main.out
if [[ ! -s main.out ]]
then
  echo " "
...
./main.out 2>stderr.log 1>/dev/null
if [[ -s stderr.log ]]
then 
  echo " "
...
echo "########## Build with g++ ##########"
g++ -std=c++20 -pedantic -Werror -Wextra -fsanitize=address -fsanitize=undefined test.cpp -o main.out
if [[ ! -s main.out ]]
then
  echo " "
...
echo "########## Valgrind check ##########"
valgrind --leak-check=yes --log-file=log.txt ./main.out
echo "Valgrind log:"
cat log.txt
